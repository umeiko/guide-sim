...
def surf_to_ndarray(surf:pygame.Surface, alpha=False):
    ...
    if surf.get_bitsize() == 8:
        frame_np = np.array(pygame.surfarray.pixels2d(surf))
    else:
...
def surf_gray_to_ndarray(surf:pygame.Surface):
    if surf.get_bitsize() == 8:
        frame_np = np.array(pygame.surfarray.pixels2d(surf))
    else:
        frame_np = np.array(pygame.surfarray.pixels3d(surf))
        frame_np = rgb_to_gray(frame_np)
    # frame_np = np.array(pygame.surfarray.pixels2d(surf))
...
class GuidewireEnv():
    ...
    def load_task(self, task_path:str, file_reload=True):
        ...
        if file_reload:
            ...
            self.metadata.no_angiography = \
                self.metadata.no_angiography.replace("\\", "/")
            print(os.path.join(self.dataset_path, self.metadata.no_angiography))
            print(os.path.join(self.dataset_path, self.metadata.mask_path))
            print(os.path.join(self.dataset_path, self.metadata.background_path))
            self.bg_surf = pygame.image.load(  # 动态背景改为没有造影的！
                        os.path.join(self.dataset_path, self.metadata.no_angiography))
            self.mask_surf = pygame.image.load(
                        os.path.join(self.dataset_path, self.metadata.mask_path))
            self.reff_surf = pygame.image.load(
    ...
    def render(self, _return_array=True)->np.ndarray:
        ...
        if _return_array:
            ...
            if self.display.get_size() != self.hyper_params.img_size\
                ...
                out_reff = pygame.transform.scale(self.reff_surf, self.hyper_params.img_size)
            
            out = surf_to_ndarray(out)
            out_reff = surf_gray_to_ndarray(out_reff)
